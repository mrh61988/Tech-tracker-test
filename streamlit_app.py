...
if time_file and ops_file:
    try:
        ...
        # FIXED: Removed 'time' from exclusion list to ensure 'timestamp' columns are correctly evaluated.
        priority_date_tiers = [
        ...
        daily_route['Drive %'] = (daily_route['Drive_Time_Hrs'] / daily_route['Total_Job_Time_Hours'].replace(0, np.nan) * 100).fillna(0.0)
        ...
        final_df['Rev_Per_Clocked_Hr'] = (final_df['Total_Assigned_Revenue'] / final_df['Total_Weekly_Clocked_Hrs'].replace(0, np.nan)).fillna(0.0)
        ...
        final_df['Daily_Avg_Diff_Hrs'] = (final_df['Total_Weekly_Diff_Hrs'] / final_df['Days_Worked'].replace(0, np.nan)).fillna(0.0)
        ...
        final_df['Simple Installs Eff'] = (final_df['Simple_Installs_Hrs'] / final_df['Assumed_LSI_Clocked'].replace(0, np.nan) * 100).fillna(0.0)
        final_df['Water Heaters Eff'] = (final_df['Water_Heaters_Hrs'] / final_df['Assumed_WH_Clocked'].replace(0, np.nan) * 100).fillna(0.0)
        ...
        ops_df['Job_Time_Weight'] = (ops_df['Total_Job_Time_Hours'] / ops_df['Tech_Total_Work_Hrs'].replace(0, np.nan)).fillna(0.0)
        ...
        cc_matrix['Cost Ratio % vs Rev'] = (cc_matrix['Combined_Cost_Total_Raw'] / cc_matrix['Gross_Invoiced_Raw'].replace(0, np.nan) * 100).fillna(0.0)
        cc_matrix['Cost Ratio % vs Rev'] = cc_matrix['Cost Ratio % vs Rev'].apply(lambda x: f"{x:.1f}%")
        
        cc_matrix['Net_Profit_Pct_Raw'] = (cc_matrix['Net_Profit_Total_Raw'] / cc_matrix['Gross_Invoiced_Raw'].replace(0, np.nan) * 100).fillna(0.0)
        ...
        bu_summary_df['Total Efficiency_Raw'] = (final_df['Total_Weekly_Job_Hrs'] / final_df['Total_Weekly_Clocked_Hrs'].replace(0, np.nan) * 100).fillna(0.0)
        bu_summary_df['Total Efficiency'] = bu_summary_df['Total Efficiency_Raw'].apply(lambda x: f"{x:.1f}%")
        bu_summary_df['Total Unallocated Hours'] = final_df['Total_Weekly_Diff_Hrs'].apply(format_hm)
        bu_summary_df = bu_summary_df.drop(columns=['Total Efficiency_Raw'])
        ...
        with tabs[0]:
            ...
            global_macro_bu_key = st.selectbox("Filter Financial Performance Dashboard By Line of Business:", ["All Sectors", "Lowes - Water Heaters", "Lowes - Simple Installs"], index=0, key="global_macro_bu_filter")
            
            df_dash_kpi = df_macro_pay.copy()
            if global_macro_bu_key != "All Sectors":
                df_dash_kpi = df_dash_kpi[df_dash_kpi['Business Unit'] == global_macro_bu_key]
            ...
            with dash_metric_col1:
                st.metric(label=f"Gross Invoiced Volume ({global_macro_bu_key})", value=f"${kpi_gross_volume:,.2f}")
            with dash_metric_col2:
                st.metric(label=f"Assumed Total Pay ({global_macro_bu_key})", value=f"${kpi_labor_payload:,.2f}")
            with dash_metric_col3:
                st.metric(label=f"Labor Pay Ratio ({global_macro_bu_key})", value=f"{kpi_pay_ratio:.1f}%")
            ...
            bu_financial_matrix['Pay % of Revenue_Raw'] = (bu_financial_matrix['Assumed Pay Raw'] / bu_financial_matrix['Gross Invoiced Revenue Raw'].replace(0, np.nan) * 100).fillna(0.0)
            bu_financial_matrix['Pay % of Revenue'] = bu_financial_matrix['Pay % of Revenue_Raw'].apply(lambda x: f"{x:.1f}%")
            ...
            with m_col2:
                ...
                rev_per_hour_df['Pay Pct'] = (rev_per_hour_df['Assumed Pay Amount'] / rev_per_hour_df['Total_Assigned_Revenue'].replace(0, np.nan) * 100).fillna(0.0)
                ...
                rev_per_hour_df['Margin per Clocked Hour Raw'] = (rev_per_hour_df['Net Margin Raw'] / rev_per_hour_df['Total_Weekly_Clocked_Hrs'].replace(0, np.nan)).fillna(0.0)
            ...
            if not df_prof_totals.empty:
                ...
                if not df_prof_filtered.empty:
                    df_prof_filtered['Profit Margin %'] = (df_prof_filtered['Net_Profit_Raw'] / df_prof_filtered['Total Invoice Amount'].replace(0, np.nan) * 100).fillna(0.0)
        ...
        with tabs[10]:
            ...
            if "🔄 The Context-Switching Penalty Alert" in test_choices:
                ...
                if 'Business Unit' in ops_df.columns:
                    ...
                    daily_merged['Avg Job Time'] = (daily_merged['Total_Job_Time_Hours'] / daily_merged['Job_Count'].replace(0, np.nan)).fillna(0.0)
            ...
            if "🗺️ Revenue Yield per Drive Hour (Geo-Routing Efficiency)" in test_choices:
                ...
                route_eff['Rev per Drive Hour Raw'] = (route_eff['Total_Revenue'] / route_eff['Total_Drive_Hrs'].replace(0, np.nan)).fillna(0.0)
                ...
                route_eff['Revenue per Drive Hour'] = route_eff['Rev per Drive Hour Raw'].apply(lambda x: f"${x:,.2f}/hr" if x > 0 else "-")
            ...
            if "🦺 Multi-Tech Labor Yield vs. Solo Runs" in test_choices:
                ...
                summary_yield['Avg Revenue per Job'] = (summary_yield['Total_Revenue'] / summary_yield['Job_Count'].replace(0, np.nan)).fillna(0.0)
                summary_yield['Revenue per Man-Hour'] = (summary_yield['Total_Revenue'] / summary_yield['Total_Man_Hours'].replace(0, np.nan)).fillna(0.0)
            ...
            if "📅 Lowe's Store Staging Delays by Day of the Week" in test_choices:
                ...
                if not store_delay_df.empty:
                    ...
                    staging_agg['Avg Delay per Visit Raw'] = (staging_agg['Total_Hours'] / staging_agg['Total_Visits'].replace(0, np.nan)).fillna(0.0)
            ...
            if "🗺️ Geographic Revenue Yield per Drive Hour" in test_choices:
                ...
                if 'Location Address' in df_yield.columns:
                    ...
                    geo_yield['Yield per Travel Hour Raw'] = (geo_yield['Gross_Invoiced_Volume'] / geo_yield['Total_Travel_Time'].replace(0, np.nan)).fillna(0.0)
